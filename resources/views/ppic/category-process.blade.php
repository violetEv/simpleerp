        ...
        <div class="max-w-7xl mx-auto">
                    ...
                    <form action="{{ route('ppic.category') }}" method="GET" class="flex items-center gap-2">
                        <div class="relative">
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Search category process..."
                        ...
                        @if ($categories->count())
                            @foreach ($categories as $categoryProcess)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap">{{ $categoryProcess->name }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        <form action="{{ route('ppic.category.delete', $categoryProcess->id) }}"
                                            ...
                                            <button type="submit" title="Delete"
                                                onclick="return confirm('Are you sure you want to delete {{ $categoryProcess->name }}?')">
            ...
            <div id="addModal" class="hidden fixed inset-0 z-50 bg-gray-600 bg-opacity-50 items-center justify-center">
                ...
                <div class="bg-white rounded-xl shadow-lg w-full max-w-md mx-4 overflow-hidden">
                    ...
                    {{-- FORM --}}
                    <form id="crud-form" action="{{ route('ppic.category.store') }}" method="POST" class="p-5">
                        @csrf
